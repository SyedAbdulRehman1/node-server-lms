...
class AuthService {
  register(email, password) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      yield (0,
      sendEmail_1.sendEmail)(registerDto.email, "Activate your account", activationLink);
      return {
  ...
  updateUser(_a) {
    return __awaiter(
      ...
      arguments,
      void 0,
      function* ({ id, oldPassword, newPassword, name, email, image }) {
        // Fetch user from DB
        const user = yield prisma.user.findUnique({
          where: { id },
        });
        ...
        if (newPassword) {
          ...
          if (user.password) {
            const isOldPasswordValid = yield bcryptjs_1.default.compare(
              oldPassword,
              user.password
            );
